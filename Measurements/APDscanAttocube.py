...
#        raise Exception("Attocube scanner could not be set!")
    ...
    if live_plot == True:
        ...
        scale_factor = 10 / 45
        secax_x = ax.secondary_xaxis('top', functions=(lambda x: x * scale_factor, lambda x: x / scale_factor))
        secax_x.set_xlabel("x (µm)")
        secax_y = ax.secondary_yaxis('right', functions=(lambda y: y * scale_factor, lambda y: y / scale_factor))
        secax_y.set_ylabel("y (µm)")
