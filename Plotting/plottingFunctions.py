...
def plot_APDscan_official(filename, size=(6,4), log_scale=True, clip=False, clipNumber=1000):
    """
    Create side-by-side scatter plots for APD scan data with linear and logarithmic scales.
    ...
    x_unique = np.unique(Loc_sorted[:, 0])
    y_unique = np.unique(Loc_sorted[:, 1])
    X, Y = np.meshgrid(max(x_unique)-x_unique, y_unique)
    X, Y = 10/45 * X + 30, 10/45 * Y + 8
    # X, Y = np.meshgrid(max(x_unique)-x_unique, -min(y_unique)+y_unique)
    # X, Y = 10/0.07 * X, 10/0.07 * Y
    ...
    # Set up figure with two subplots
    fig, axes = plt.subplots(1, 1, figsize=(size[0], size[1]), constrained_layout=True)
    
    if log_scale == True:
        scatter = axes.pcolormesh(X, Y, apd_grid, cmap='Reds', norm=LogNorm(vmin=min(apd), vmax=max(apd)))
    else:
        scatter = axes.pcolormesh(X, Y, apd_grid, cmap='Reds', vmin=min(apd), vmax=max(apd))
    cbar = fig.colorbar(scatter, ax=axes, shrink=0.75) #, shrink=0.55)
    cbar.set_label('Count rate (Hz)', fontsize=20)
    cbar.ax.tick_params(labelsize=18)

    axes.set_xlabel(r'x ($\mu$m)', fontsize=20)
    axes.set_ylabel(r'y ($\mu$m)', fontsize=20)
    axes.tick_params(axis='both', labelsize=18)
    axes.set_box_aspect(aspect_ratio)  # Only scale axes
...
def plot_spectrum_official(filename, xlim=(1.7,2.3), ymax=None):
    """Create a plot for spectrum data and return the figure."""
    x = save.load_data(filename)
    wavelengths = np.array(x['wavelength'])
    signals = np.array(x['intensity'])
    ...
    wl_ticks = np.arange(540, 900, 20)
    energy_ticks_for_wl = nm_to_eV(wl_ticks)
    
    ax1.set_xticks(np.arange(xlim[0]-0.01, xlim[1], 0.05))
    ax1.set_xlim(xlim)
    ax1.tick_params(axis='both', labelsize=14)
    
    ax2.set_xticks(energy_ticks_for_wl)
    ax2.set_xticklabels([f'{e:.0f}' for e in wl_ticks], fontsize=14)
    ax2.set_xlim(xlim)
